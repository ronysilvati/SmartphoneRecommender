...
def combine_features(row):
    return row['CPU'] + ' ' + row['Storage capacity'] + ' ' + row['Removable storage'] + ' ' + row['OS'] \
...
def get_recommendations(cpu, storage_capacity, removable_storage, ram, os, battery, display, camera, fingerprint
    ...
    for element in sorted_similar_smartphones:
        if i < max_recommendations:
            content_to_return = {}
            content_to_return['model'] = df["Model"][element[0]]
            content_to_return['details'] = df["combined_features"][element[0]]

            recommendations_list.append(content_to_return)
        else:
